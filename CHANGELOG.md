# Changelog
## 0.6.2
* **The install root and `RustDedicated_Data/Managed` (with `x64` and `x86`) are now group-writable and setgid when `rust_gameserver_manager_users` is set.** 0.6.1 widened `oxide/plugins` so the panel could install plugins; a panel-driven *Oxide* install still died at `Permission denied` (rustd.xyz#575) because it backs up into the install root and extracts over `Managed`, and both were left `0755`. Directory write is the whole grant — `unzip -o` unlinks and recreates, so the game's own `644` files are untouched. What it lets the manager do is replace assemblies under `Managed`, which is what installing Oxide is
* The new `rust_gameserver_game_dir_mode` derives from whether a manager is configured, exactly like `rust_gameserver_plugins_dir_mode`. `x64`/`x86` are created ahead of steamcmd on a fresh host and corrected in place on an existing one
## 0.6.1
* **The `oxide/plugins` directory is now group-writable and setgid when `rust_gameserver_manager_users` is set.** It was created `0755` owned by the runtime user, so an external manager (rustd.xyz) in the runtime group could not write a plugin into it -- the panel's plugin install failed with a bare `Permission denied` on every server this collection has ever provisioned, and the plugins already present had been put there by Ansible rather than by the panel. The identity directory has had this treatment since 0.4.0 for the same reason; the plugins directory was simply missed
* The new `rust_gameserver_plugins_dir_mode` defaults to `2775` when manager users are configured and `0755` when they are not, rather than defaulting to the old value and requiring a second opt-in. A deployment that names a manager and then forgets the mode is exactly the bug above, and there is no case where a manager is wanted but writing plugins is not
## 0.6.0
